...
def shape(X0,Y0,key,L,capside,orkey,which_side_binds):
	Xs = [X0]
	Ys = [Y0]
	if capside:
...
def setup_2mols(L, key1, key2, capside,which_side_binds):
	x0 = -L/2.0
	y0 = -L/2.0
	dx = 1.0
	dy = 2*np.cos(np.pi/6)
	...
	X0 = xs[idx]
	Y0 = ys[idx]
	Xs, Ys, ids, types = shape(X0,Y0,key1,L,capside,'R','none,this_is_chevron')
	# Empty safe space
	dxs = xs-X0
	dys = ys-Y0
	dxs[dxs < -L/2.0] += L
	...
	nX0 = nxs[idx]
	nY0 = nys[idx]
	nXs, nYs, nids, ntypes = shape(nX0,nY0,key2,L,capside,'R',which_side_binds)
	# Assemble
...
parser.add_argument('-side', '--which_side_binds', help='if it binds from back or tip - string either B or T - REQUIRED', required=True, type=str)
...
L = float(args.L)
which_side_binds = str(args.which_side_binds)
...
ids, types, X, Y = setup_2mols(L, 'C', 'R', True, which_side_binds)
if not len(ids) == 16:
	print()
...
''')
if eps == 0:
	if which_side_binds == 'B':
		f.write("pair_coeff          3 9 cosine/squared 1.00 ${rdist} ${rdist} wca 			#eps interactions \n")
		f.write("pair_coeff          5 13 cosine/squared 1.00 ${rdist} ${rdist} wca 			\n")
	elif which_side_binds == 'T':
		f.write("pair_coeff          2 10 cosine/squared 1.00 ${rdist} ${rdist} wca 	    #eps interactions \n")
		f.write("pair_coeff          6 12 cosine/squared 1.00 ${rdist} ${rdist} wca 			\n")
else:
	if which_side_binds == 'B':
		f.write("pair_coeff          3 9 cosine/squared ${eps} 1.00 ${coff} wca  			#eps interactions \n")
		f.write("pair_coeff          5 13 cosine/squared ${eps} 1.00 ${coff} wca  			\n")
	elif which_side_binds == 'T':
		f.write("pair_coeff          2 10 cosine/squared ${eps} 1.00 ${coff} wca 			#eps interactions \n")
		f.write("pair_coeff          6 12 cosine/squared ${eps} 1.00 ${coff} wca  			#eps interactions \n")
if epsC == 0:
	f.write("pair_coeff          1 4 cosine/squared 1.00 ${rdist} ${rdist} wca 			# chevron shape pair intra 1-4 for both monomers\n")
	f.write("pair_coeff          8 11 cosine/squared 1.00 ${rdist} ${rdist} wca 			\n")
	if which_side_binds == 'B':
		f.write("pair_coeff          4 8 cosine/squared 1.00 ${rdist} ${rdist} wca 			# epsC interaction between monomers \n")
	elif which_side_binds == 'T':
		f.write("pair_coeff          1 11 cosine/squared 1.00 ${rdist} ${rdist} wca 		# epsC interaction between monomers \n")
else:
	f.write("pair_coeff          1 4 cosine/squared ${epsC} 1.00 ${coff} wca 			# chevron shape pair intra 1-4 for both monomers\n")
	f.write("pair_coeff          8 11 cosine/squared ${epsC} 1.00 ${coff} wca 			\n")
	if which_side_binds == 'B':
		f.write("pair_coeff          4 8 cosine/squared ${epsC} 1.00 ${coff} wca 			# epsC interaction between monomers\n")
	elif which_side_binds == 'T':
		f.write("pair_coeff          1 11 cosine/squared ${epsC} 1.00 ${coff} wca 			# epsC interaction between monomers\n")
if epsT == 0:
	f.write("pair_coeff          3 5 cosine/squared 1.00 ${rdist} ${rdist} wca 			# triangle shape pair intra 3-5 for both monomers\n")
	f.write("pair_coeff          10 12 cosine/squared 1.00 ${rdist} ${rdist} wca 		\n")
else:
	f.write("pair_coeff          3 5 cosine/squared ${epsT} 1.00 ${coff} wca 			# triangle shape pair intra 3-5 for both monomers\n")
	f.write("pair_coeff          10 12 cosine/squared ${epsT} 1.00 ${coff} wca 			\n")
f.write('''
...
if which_side_binds == 'T':
	f.write('''fix                 fHaltT all halt 1 v_vTI > 0 error continue\n''')
elif which_side_binds == 'B': 
	f.write('''fix                 fHaltB all halt 1 v_vBI > 0 error continue\n''')
...
if which_side_binds == 'T':
	f.write('''fix                 fHaltT all halt 1 v_vTI == 2 error continue\n''')
elif which_side_binds == 'B': 
	f.write('''fix                 fHaltB all halt 1 v_vBI == 2 error continue\n''')
