  ...
  probability(nullptr), created(nullptr), copy(nullptr), random(nullptr), list(nullptr)
{
  ...
  constrainflag = 0;
  constrainpass = 0;
  imflag = 0;                                                                   // declaring imflag to 0 -- imflag variable flags for only inter-molecular bonds -- Christian Vanhille Campos - 09/06/2023
  amin = 0;
  amax = 180;
  ...
  while (iarg < narg) {
    if (strcmp(arg[iarg],"iparam") == 0) {
      ...
      constrainflag = 1;
      iarg += 3;
    } else if (strcmp(arg[iarg],"inter_mol") == 0) {                            // reading imflag argument (inter_mol) -- imflag variable flags for only inter-molecular bonds -- Christian Vanhille Campos - 09/06/2023
      imflag = 1;
      iarg += 1;
...
{
  int i,j,k,m,n,ii,jj,inum,jnum,itype,jtype,n1,n2,n3,possible,imolecule,jmolecule;  // added declaration of molecule ID ints for atoms i and j, for inter-molecular bonds only implementation -- Christian Vanhille Campos - 09/06/2023
  ...
  int *mask = atom->mask;
  int *type = atom->type;
  tagint *molecule = atom->molecule;                                            // getting molecule IDs of the atoms for inter-molecular bonds only implementation -- Christian Vanhille Campos - 09/06/2023
  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    jlist = firstneigh[i];
    jnum = numneigh[i];
    imolecule = molecule[i];                                                  // molecule ID of atom i, for inter-molecular bonds only implementation -- Christian Vanhille Campos - 09/06/2023
    ...
    for (jj = 0; jj < jnum; jj++) {
      ...
      jtype = type[j];
      jmolecule = molecule[j];                                                // molecule ID of atom j, for inter-molecular bonds only implementation -- Christian Vanhille Campos - 09/06/2023

      if (imflag && imolecule == jmolecule) {continue;}                       // if inter-molecular bonds flag on and both atoms have the same molecule ID skip this pair -- Christian Vanhille Campos - 09/06/2023
